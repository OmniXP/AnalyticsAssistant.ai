...
/** Optional CSV for products */
function downloadCsvProducts(rows, startDate, endDate) {
  if (!rows?.length) return;
  const header = ["Item", "ID", "Items Viewed", "Add-to-Carts", "Items Purchased", "Item Revenue (GBP)"];
  const lines = rows.map((r) => [
    r.name,
    r.id,
    r.itemsViewed,
 ...
 * Reusable AI block
 * ======================= */
function AiBlock({ label = "Summarise with AI", endpoint, payload, compact = false }) {
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div style={{ display: "inline-flex", gap: 8, alignItems: "center" }}>
      <button onClick={run} style={{ padding: compact ? "6px 10px" : "10px 14px", cursor: "pointer" }} disabled={loading}>
        {loading ? "Summarising…" : label}
      </button>
      <button onClick={copy} style={{ padding: compact ? "6px 10px" : "10px 14px", cursor: "pointer" }} disabled={!text}>
...
function EcommerceKPIs({ propertyId, startDate, endDate }) {
  ...
  const summarise = async () => {
    ...
    try {
      ...
      let data = null; try { data = raw ? JSON.parse(raw) : null; } catch {}
      if (!res.ok) throw new Error((data && (data.error || data.message)) || raw || `HTTP ${res.status}`);
      setAiText((data && data.summary) || raw || "No response");
    } catch (e) {
  ...
  return (
    <section style={{ marginTop: 32 }}>
      ...
      {totals && (
        <div style={{ marginTop: 12 }}>
          <ul style={{ marginTop: 0 }}>
            <li><b>Revenue:</b> GBP {Number(totals.revenue || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</li>
            <li><b>Purchases:</b> {Number(totals.purchases || 0).toLocaleString()}</li>
            <li><b>Sessions:</b> {Number(totals.sessions || 0).toLocaleString()}</li>
            <li><b>Users:</b> {Number(totals.users || 0).toLocaleString()}</li>
            <li><b>Transactions / Session:</b> {(Number(totals.purchases || 0) / Math.max(1, Number(totals.sessions || 0))).toFixed(4)}</li>
          </ul>
          {aiText && (
            <div style={{ marginTop: 12, background: "#fffceb", border: "1px solid #f5e08f", padding: 12, borderRadius: 6, whiteSpace: "pre-wrap" }}>
              {aiText}
      ...
      {!totals && !loading && !error && <p style={{ marginTop: 8, color: "#666" }}>No KPIs loaded yet.</p>}
      {aiError && <p style={{ color: "crimson", marginTop: 8 }}>Error: {aiError}</p>}
    </section>
  );
}
...
function Products({ propertyId, startDate, endDate }) {
  ...
  return (
    <section style={{ marginTop: 32 }}>
      ...
      {error && <p style={{ color: "crimson", marginTop: 12, whiteSpace: "pre-wrap" }}>Error: {error}</p>}
      {note && !error && <p style={{ marginTop: 8, color: "#666" }}>{note}</p>}
          ...
          <table style={{ borderCollapse: "collapse", width: "100%" }}>
            <thead>
              <tr>
                ...
                <th style={{ textAlign: "right", borderBottom: "1px solid #ddd", padding: 8 }}>Item Revenue (GBP)</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
      ...
      {rows.length === 0 && !loading && !error && !note && <p style={{ marginTop: 8, color: "#666" }}>No rows loaded yet.</p>}
    </section>
  );
}
...
function CheckoutFunnel({ propertyId, startDate, endDate }) {
  ...
  return (
    <section style={{ marginTop: 32, marginBottom: 48 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <h3 style={{ margin: 0 }}>Checkout funnel (unique users per step)</h3>
        <button onClick={load} style={{ padding: "8px 12px", cursor: "pointer" }} disabled={loading || !propertyId}>
