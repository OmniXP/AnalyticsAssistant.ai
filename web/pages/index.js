...
function downloadCsvProducts(rows, startDate, endDate) {
  if (!rows?.length) return;
  const header = ["Item", "ID", "Views", "Add-to-Carts", "Items Purchased", "Item Revenue"];
  const lines = rows.map((r) => [
    r.name,
    r.id || "",
    r.itemsViewed,
  ...
  const csv = [header, ...lines]
    .map((cols) => cols.map((v) => `"${String(v).replace(/"/g, '""')}"`).join(","))
    .join("\n");
  const filename = `products_${startDate}_to_${endDate}.csv`;
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
...
/** QuickChart pie chart URL (channels) */
...
export default function Home() {
  ...
  // Shared GA4 fetcher for the channel overview
  ...
  return (
    ...
    >
      ...
      <p style={{ marginTop: 0, color: "#555" }}>
        Connect GA4, choose a date range, and view traffic by channel.
      </p>
...
function AiInline({ endpoint, payload, disabled }) {
  ...
  return (
    <div style={{ display: "inline-flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
      ...
      {error && (
        <span style={{ color: "crimson", marginLeft: 6, whiteSpace: "pre-wrap" }}>
          Error: {error}
        </span>
      )}
      {text && (
...
function TopPages({ propertyId, startDate, endDate }) {
  ...
  const load = async () => {
    setLoading(true); setError(""); setRows([]); setHasLoaded(true);
    try {
  ...
  return (
    <section style={{ marginTop: 32 }}>
      ...
      {!hasRows && !error && hasLoaded && <p style={{ marginTop: 8, color: "#666" }}>No rows loaded yet.</p>}
...
function EcommerceKPIs({ propertyId, startDate, endDate }) {
  ...
  const load = async () => {
    setLoading(true); setError(""); setTotals(null); setHasLoaded(true);
    try {
  ...
  return (
    <section style={{ marginTop: 32 }}>
      ...
      {error && <p style={{ color: "crimson", marginTop: 12, whiteSpace: "pre-wrap" }}>Error: {error}</p>}
      {!hasTotals && !error && hasLoaded && (
        <p style={{ marginTop: 8, color: "#666" }}>No totals for this date range.</p>
      )}

      {hasTotals && (
        <div style={{ marginTop: 12 }}>
          <ul style={{ marginTop: 0 }}>
            <li><b>Revenue:</b> {(totals.revenue ?? 0).toLocaleString(undefined, { style: "currency", currency: "GBP" })}</li>
            <li><b>Transactions:</b> {(totals.transactions ?? 0).toLocaleString()}</li>
            <li><b>Purchaser conversion rate:</b> {(totals.purchaserRate ?? 0).toFixed(2)}%</li>
            <li><b>Average order value:</b> {(totals.aov ?? 0).toLocaleString(undefined, { style: "currency", currency: "GBP" })}</li>
            <li><b>Sessions:</b> {(totals.sessions ?? 0).toLocaleString()}</li>
            <li><b>CTR (if available):</b> {typeof totals.ctr === "number" ? `${totals.ctr.toFixed(2)}%` : "—"}</li>
          </ul>

          {/* Optional in-section AI block (kept for parity) */}
          <div style={{ marginTop: 8, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <button onClick={summarise} style={{ padding: "8px 12px", cursor: "pointer" }} disabled={!hasTotals || aiLoading}>
              {aiLoading ? "Summarising…" : "Summarise (alt)"}
            </button>
            <button onClick={copy} style={{ padding: "8px 12px", cursor: aiText ? "pointer" : "not-allowed" }} disabled={!aiText}>
              {copied ? "Copied!" : "Copy insight"}
            </button>
            {aiError && <span style={{ color: "crimson", whiteSpace: "pre-wrap" }}>Error: {aiError}</span>}
            {aiText && (
              <span
                style={{
                  background: "#fffceb",
